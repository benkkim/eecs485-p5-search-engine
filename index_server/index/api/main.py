...
def init():
    ...
    with open("index_server/index/stopwords.txt", "r") as file:
        global SW
        SW = set([line.strip() for line in file.readlines()])

    with open("index_server/index/pagerank.out", "r") as file:
        global PR
...
def get_hits():
    ...
    # clean query
    q = q.split()
    q = [re.sub(r"[^a-zA-Z0-9]+", '', word.strip()) for word in q]
    q = [word.lower() for word in q]
    q = [word for word in q if word not in SW]
    print(q)

    # open the index file
    term_dict= {}
    with open(Path("index_server/index/inverted_index")/Path(index.app.config["INDEX_PATH"]), "r") as file:
        lines = file.readlines()
        for line in lines:
            line = line.split()
            line = [word.strip() for word in line]
    ...
    for doc in doc_dict:
        ...
        num = scores_dict[doc]
        tf_idf = num/den
        weight = w * PR[doc_id] + (1-w) * tf_idf
        pr_score[doc_id] = weight

    context = {}
    context["hits"] = []
    #sort pr_score by value
    pr_score = {k: v for k, v in sorted(pr_score.items(), key=lambda item: item[1], reverse=True)}
    for doc in pr_score:
        context["hits"].append({
            "docid": doc,
            "score": pr_score[doc]
